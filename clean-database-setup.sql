-- Clean database setup for Echocity - NO RLS policy conflicts
-- Run this to replace any existing setup

-- First, drop existing policies that might cause conflicts
...
DROP POLICY IF EXISTS "user_roles_policy" ON public.profiles;
...
-- Enable RLS with SIMPLE policies (no recursion risk)
ALTER TABLE public.profiles ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.categories ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.departments ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.complaints ENABLE ROW LEVEL SECURITY;

-- SIMPLE RLS policies - public read access for now
CREATE POLICY "profiles_select_policy" ON public.profiles FOR SELECT USING (true);
CREATE POLICY "categories_select_policy" ON public.categories FOR SELECT USING (true);
CREATE POLICY "departments_select_policy" ON public.departments FOR SELECT USING (true);
CREATE POLICY "complaints_select_policy" ON public.complaints FOR SELECT USING (true);
...
CREATE POLICY "complaints_insert_policy" ON public.complaints FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "complaints_update_policy" ON public.complaints FOR UPDATE USING (auth.uid() = user_id);
